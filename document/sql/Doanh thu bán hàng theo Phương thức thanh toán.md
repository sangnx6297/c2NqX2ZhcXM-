Query: Doanh thu bán hàng theo Phương thức thanh toán
```SQL
...
-- 	`base_o`.`order_sell_chanel_code`,
	...
	SUM( IF(`base_o`.`payment_method` = 'PAY001', success_o.order_total_exclude_vat, 0) ) AS 'COD',
	SUM( IF(`base_o`.`payment_method` <> 'PAY001', success_o.order_total_exclude_vat, 0) ) AS 'Chuyển khoản'
...
FROM
	...
	SELECT
		...
		`channel_name`,
		MD5(
		CONCAT_WS( '#', order_sell_chanel_code, order_sell_chanel_name, channel_code, channel_name, payment_method )) AS keymap 
	FROM
	...
	GROUP BY
		...
		`channel_name`,
		MD5(
		CONCAT_WS( '#', order_sell_chanel_code, order_sell_chanel_name, channel_code, channel_name, payment_method ))) `base_o`
	LEFT JOIN (
	SELECT
		MD5(
		CONCAT_WS( '#', oh.order_sell_chanel_code, oh.order_sell_chanel_name, oh.channel_code, oh.channel_name,payment_method )) AS keymap,
	...
	FROM
		...
		GROUP BY
			...
			`channel_name`,
			MD5(
			CONCAT_WS( '#', order_sell_chanel_code, order_sell_chanel_name, channel_code, channel_name,payment_method )),
			`order_code`,
			`order_id`,
			`order_updated_status_code`,
			`order_sell_chanel_code`,
	...
	GROUP BY
		MD5(
		CONCAT_WS( '#', oh.order_sell_chanel_code, oh.order_sell_chanel_name, oh.channel_code, oh.channel_name,payment_method ))) `return_failed_to_o` ON return_failed_to_o.keymap = base_o.keymap
	LEFT JOIN (
	SELECT
		MD5(
		CONCAT_WS( '#', oh.order_sell_chanel_code, oh.order_sell_chanel_name, oh.channel_code, oh.channel_name,payment_method )) AS keymap,
	...
	FROM
		...
		SELECT
			...
			`channel_name`,
			MD5(
			CONCAT_WS( '#', order_sell_chanel_code, order_sell_chanel_name, channel_code, channel_name, payment_method,payment_method )) AS keymap,
			`order_code`,
			`order_id`,
			`order_updated_status_code`,
			`order_updated_date`,
